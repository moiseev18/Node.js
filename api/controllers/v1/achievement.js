...
var self =  {
    ...
    find: function(req, res) {
        ...
        elastic.search({
            index: elasticService.INDEX_ACHIEVEMENTS,
            ...
            if(err){
                ...
                meta.statusCode = 404;
                meta.msg = "No achievement found for your query";
                meta.response_time = ((Date.now() - startTime)/1000)+'s';
                res.status(meta.statusCode).json(service.singleDataResponse(achievement,meta));
            }else{
                achievement =  response.hits;
                meta.statusCode = 200;
                meta.response_time = ((Date.now() - startTime) / 1000) + 's';
                res.json(200, service.singleDataResponse(achievement, meta));
            }
        });
    },
    create: function (req, res, next) {
        ...
        elastic.create({
            ...
            if(err) {
                meta.statusCode = 503;
                meta.msg = "'cannot save achievement to elastic'";
                meta.extra = err.errors;
    ...
    findById: function (req, res, next) {
        ...
        elastic.get({
            index: elasticService.INDEX_ACHIEVEMENTS,
            type: elasticService.DOCUMENT_MAIN,
            id: req.params.achievement_id
        }).then(
            function(resp) {
                achievement =  resp._source;
                achievement.id=resp._id;
                meta.statusCode = 200;
                meta.response_time = ((Date.now() - startTime) / 1000) + 's';
                res.json(200, service.singleDataResponse(achievement, meta));
            },
    ...
    update: function (req, res, next) {
        ...
        elastic.update({
            ...
            if(err){
                ...
                meta.statusCode = 404;
                meta.mssg = "Could not update achievement";
                meta.response_time = ((Date.now() - startTime)/1000)+'s';
                res.status(meta.statusCode).json(service.singleDataResponse(achievement,meta));
            }
            else{
    ...
    destroy: function (req, res, next) {
        ...
        console.log('id  : ',req.params.achievement_id);
